"""
AI Hub 정면/측면 이미지의 base-id 그룹 중복(leakage) 점검
==============================================================
배경: data_matching.py가 AI Hub 라벨을 정면(TL/VL_아토피_정면)과 측면
(TL/VL_아토피_측면) 폴더에서 각각 수집해 하나의 학습 샘플 목록으로 합치고,
data_split.py는 그 목록을 이미지 단위로 완전 랜덤 7:2:1 분할한다(base-id
그룹 분할 아님). 동일 base-id로 추정되는 정면·측면 두 장이 있다면, 그 두 이미지가
...
⚠️ 실행 전 필수: STEP 0 스키마 확인 결과를 먼저 눈으로 확인할 것.
    ...
    "정면·측면 두 이미지가 동일 base-id임을 알려주는 필드"가
...
def inspect_schema(n_samples=3):
    ...
    print("\n" + "=" * 70)
    print("⚠️ 위 출력에서 '정면 파일과 측면 파일이 동일 base-id임을 나타내는 필드'가")
...
# ============================================================
# STEP 2: identifier에서 "정면/측면 공통 접두어(추정 base-id)" 추출 시도
# 예) identifier가 "P0001_front"/"P0001_side" 같은 패턴이면 "_front"/"_side" 등
# 접미어를 떼면 동일 base-id로 추정되는 것끼리 묶일 수 있음. 정확한 규칙은 STEP 0 확인 후 조정할 것.
# ============================================================
...
def analyze_subject_overlap(records):
    print("\n" + "=" * 70)
    print("STEP 2: identifier 기준 정면/측면 base-id 그룹 중복 분석")
    print("=" * 70)
    ...
    print(f"\n[추정 규칙 기준] 정면+측면 둘 다 가진 것으로 추정되는 base-id 그룹 수: {len(multi_view_subjects)}")
...
# ============================================================
# STEP 3: 실제 data_split.py 로직을 재현해서, 같은 base-id 그룹의 이미지가
...
def simulate_split_and_check_leakage(records):
    ...
    print("\n" + "=" * 70)
    print("STEP 3: data_split.py와 동일한 랜덤 분할 재현 후 base-id 그룹 중복 확인")
    print("=" * 70)
    ...
    print(f"Train base-id 그룹 수: {len(train_subj)} / Val: {len(val_subj)} / Test: {len(test_subj)}")
    print(f"Train∩Val 중복 base-id 그룹: {len(leak_train_val)}")
    print(f"Train∩Test 중복 base-id 그룹: {len(leak_train_test)}  ← 이게 제일 중요 (모델이 학습 중 본 것과 동일 base-id로 추정되는 이미지를 테스트에서 다시 봄)")
    print(f"Val∩Test 중복 base-id 그룹: {len(leak_val_test)}")

    total_leaked_subjects = len(leak_train_val | leak_train_test | leak_val_test)
    total_subjects = len(set(subjects))
    print(f"\n[핵심 숫자] 전체 base-id 그룹 {total_subjects}개 중 "
          f"split 간 중복된 그룹: {total_leaked_subjects}개 "
...
if __name__ == "__main__":
    ...
    if result["train_test_leak"] > 0:
        print(f"⚠️ Train/Test 간 base-id 그룹 중복 {result['train_test_leak']}개 확인 — "
              f"실제 leakage입니다. GroupShuffleSplit으로 base-id 그룹 보존 split 재구성 필요.")
    else:
        print("✅ 이 실행에서는 Train/Test 간 base-id 그룹 중복이 발견되지 않았습니다. "
