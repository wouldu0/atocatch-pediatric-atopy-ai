...
AtoCatch는 "아토피가 언제, 누구에게 가장 필요한 서비스인가"라는 질문에서 출발했습니다. 아토피 피부염은 대부분 생애 초기에 발병하는 것으로 알려져 있어 조기에 위험 요인을 파악하는 것의 가치가 크다고 판단해, 서비스 대상을 영유아로 구체화했습니다. 실제로 국내 아토피 환자 중에서도 0~4세가 13.5만 명으로 가장 많고, 방치 시 1인당 의료비는 20대 기준 7배까지 늘어납니다.

발병 전 시점의 환경·유전 요인이 이후 실제 발병으로 이어지는지 확인하려면 같은 아동을 여러 시점에 걸쳐 추적한 종단 패널 데이터가 필요했고, 이 기준으로 설문 위험도 모델의 데이터를 한국아동패널(PSKC)로 선정했습니다. AtoCatch는 이 설문 데이터와 피부 이미지 분석 모델을 결합해, 영유아 아토피의 위험 신호를 조기에 확인하고 관리하도록 돕는 AI 헬스케어 서비스입니다.
