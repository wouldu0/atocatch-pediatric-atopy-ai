...
| 원본 다변량 로지스틱 회귀의 유의 위험요인(p<0.05) | 항생제 3회↑(OR 1.96) · 부모 아토피(1.72) · 부모 비염(1.39) · 실내 곰팡이(1.37) |
...
<details>
<summary><b>🧹 프로젝트 종료 후 검증·리팩터링</b></summary>

프로젝트 종료 후 포트폴리오 정리 과정에서 기존 코드와 평가 절차를 다시 점검했습니다. 단순 코드 정리보다 **실제 서비스 예측 오류와 평가 신뢰성에 영향을 주는 문제를 우선적으로 수정**했습니다.

- **설문 예측 버그 수정**: 8개 설문 문항의 예/아니오 인코딩이 학습 데이터와 반대로 입력되던 문제와 항생제 응답 카테고리의 off-by-one 오류를 수정해 실제 서비스 예측 오류를 바로잡았습니다.
- **Binary 평가 누수 수정**: AI Hub 이미지의 동일 base-id 그룹이 train/val/test에 나뉘는 문제를 발견해 grouped split으로 재구성하고 재학습한 모델로 배포 모델을 교체했습니다.
- **IGA 평가 구조 재설계**: IGA 중증도 모델에서도 base-id leakage와 test set을 이용한 threshold 선택 문제를 확인해, validation에서 threshold를 결정하고 test는 최종 평가에만 사용하는 구조로 다시 학습했습니다.
- **IGA content-level 중복 제거**: 서로 다른 base-id로 등록됐지만 파일 내용이 완전히 동일한 이미지를 SHA-256으로 추가 탐지해 제거하고, base-id와 content 중복을 함께 고려한 그룹 기준으로 재분할·재학습했습니다.
- **설문 outcome 재검증**: 공식 코드북과 원본 데이터를 다시 대조해 추적 중단 아동의 outcome 처리에 방법론적 한계가 있음을 확인하고, 사후 재분석 결과와 한계를 README에 명시했습니다.
- **시각화 개선**: 기존 Grad-CAM이 일부 실제 이미지에서 병변과 무관한 정상 피부에 활성화되는 사례를 확인해 Grad-CAM++로 교체했습니다.
- **재현성·배포 환경 정리**: 모델 가중치의 외부 Google Drive 의존성을 제거하고, threshold·모델 설정을 config에서 동적으로 불러오도록 수정했습니다. 또한 상대경로 정리, 학습 데이터 manifest 보존, `.gitignore` 추가 등 재현 가능한 실행 환경을 정리했습니다.
...
- **설문 outcome과 추적 탈락의 한계**: 추적이 중단된 일부 아동이 기존 outcome에서 미발병으로 처리됐으며, 엄격한 기준으로 재분석하면 예측 성능이 낮아집니다. 또한 추적 탈락이 무작위로 발생했다고 확인할 수 없어 attrition bias 가능성이 남아 있습니다. 사후 재분석은 방법론 검증용이며 현재 배포 모델에는 반영하지 않았습니다.

- **위험도 구간의 해석 범위**: 화면에 표시되는 저·중·고위험 구간(0.13 / 0.20)은 사용자 이해를 돕기 위한 UX 표시 기준입니다. 모델의 실제 operating threshold(0.12)와는 별개이며 통계적으로 도출된 위험 구간은 아닙니다.

- **이미지 모델의 외부 일반화 한계**: 기존 DermNet holdout 108장은 모델 선택과 threshold 설정에도 사용되어 완전히 독립적인 test set은 아닙니다. 이를 보완하기 위해 모델·threshold·전처리를 동결한 뒤 SkinDisNet에서 독립 external robustness evaluation을 수행했으나, 해당 데이터에는 patient ID·age·sex metadata가 없어 영유아 대상 성능으로 해석할 수 없습니다. 또한 negative 질환 구성이 기존 학습 데이터와 달라 DermNet 성능과 직접적인 전후 비교로 해석하지 않았습니다.

- **재학습 결과의 재현성 한계**: base-id 및 content-level dedup 재학습은 seed=42의 1회 실행 결과입니다. 또한 데이터 분할과 최종 test 표본 구성이 함께 변경됐기 때문에, 성능 변화가 leakage 제거 자체의 효과인지 test 구성 변화의 영향인지 분리해 단정하기 어렵습니다.

- **Grad-CAM++의 한계**: Grad-CAM++는 모델이 판단에 참고한 영역을 시각적으로 보여주는 설명 도구일 뿐 실제 병변 위치를 정확히 검출하는 모델은 아닙니다. 기존 Grad-CAM보다 병변과의 정합성이 개선됐지만 일부 이미지에서는 여전히 정상 피부에 활성화가 나타날 수 있습니다.
